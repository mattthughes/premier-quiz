...
I decided to go for this colour palette for the project, the green and red will be used when the user gets a question right or wrong. The light colours will be used for the background. The look is very simplistic to not take away from the quiz itself and the text colour will be black. 
...
I decided to go with the following font style for the headings, this felt like a professional style and did no take much away from the overall site as it didnt look out of place.
...
* This element will be generated by the javascript object question, where these questions will randomise upon starting the quiz giving returning users a different experience each time.
...
* This element will be generated when clicking on the instructions button. This will be done by using the class list method to add and remove which will keep all elements on the same page. I will be using these two methods to trigger which elements are shown and which are hidden at any given time. Allowing the user to view the insturctions first, will allow users to understand what they need to do before starting the quiz, in turn welcoming new users instead of detering them.
...
* This element will allow users to decide when they would like to start the quiz, rather than the first question loading upon opening the website. This allows users to take their time and will not force users to start the quiz straight away.
...
* This element is the main purpose of the quiz. These answers will be generated by the anwers obje which is a child of the questions object. It will be generated by the function showQuestions() and randomised using the function shuffleQuestions(array) .
...
* This element will be the main purpose behind the quiz, which will give users a visual que showing them whether they got the answers correct or not which will be kept track of and trigged by javascript.
...
* This element will allow the user to progress to the next question and will only show up when the question has been answered allowing the user to take their time with answering the questions.
...
* This element wil be shown to users if they get every question correct. The contact page will allow the user -via a drop down- to select there favourite team, enter their email and first and surname as well. Once submitting this form it will load an extra page thanking the user for there participation in the quiz, while also entering them into a raffel for a chance to visit a live Premier league game of there choosing.
...
* This element will only be shown to the users that have got all 10 questions correct and have completed the raffle. This will tell the user what data they have inputted, adding more interactivity to the quiz while also giving the user a back button to play the quiz again.
...
* Sorting array function inspired by W3 schools and was customised to meet my projects needs, sorting array documentation. [Sorting array function example](https://www.w3schools.com/js/js_array_sort.asp)

* Validation examples function inspired by W3 schools validation and customised to meet my projects needs. [Form Validation](https://www.w3schools.com/js/js_validation.asp)
